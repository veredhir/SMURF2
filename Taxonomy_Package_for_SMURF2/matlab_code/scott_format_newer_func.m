...
for sct = 1:length(scott_levels_2save)
    ...
    for nn = 1:length(batch_directories_list)
        ...
        sl2_ind = find(batch_directories_list{nn}=='/',1,'last');
        nn_sample_name = batch_directories_list{nn}(sl2_ind+1:end);
        sl1_ind = find(batch_directories_list{nn}(1:sl2_ind-1)=='/',1,'last');
        nn_batch_name = batch_directories_list{nn}(sl1_ind+1:sl2_ind-1);
        ...
        if ~reduce_sample_name
            samples_names{nn} = [nn_batch_name '/' nn_sample_name];
        else
