...
class EmirgeIteration(object):
    ...
    def init(self, prev_emirge_iteration):
        ...
        curr_state_df = pd.read_csv(prev_emirge_iteration.paths.current_state, index_col=False)
        ...
        logging.debug("ref columns = {}".format(reference_df.columns))
        ...
        posteriors_df = pd.read_csv(prev_emirge_iteration.paths.posteriors, index_col=False)
        posteriors_df.to_csv(self.paths.posteriors, index=False)
        reads_df = pd.read_csv(prev_emirge_iteration.paths.mapping,
    ...
    def _get_region_using_primer(self, read, minimum_match_score=0.7):
        match_size = self.primers.min_size
    ...
    def _find_mapping(self, unique_reads_df, full_ref_df=None):
        ...
        for region, reads_df in unique_reads_for_region_groups:
            ...
            logging.debug("old columns = {}, wanted columns = {}".format(reads_and_refs_df.columns, MappingForamt.full_header))
    ...
    def prepare_reads(self, fastq_path, reversed_fastq_path, read_len, unique_group_size_fraction=100000):
        ...
        for (record, reversed_record) in izip(SeqIO.parse(fastq_path, "fastq"),
            ...
            if region is not None:
                ...
                read_index += 1
            all_reads_counter += 1
        logging.info("Reads with matched region found = {} out of {} reads, short_reads={}".format(read_index, all_reads_counter, short_reads))
    ...
    def calc_prob_N(self):
        ...
        for ref_group_id, mapping_ref_df in mapping_grouped_by_ref:
            ...
            memory_usage = ref_full_data.memory_usage(index=True, deep=True).sum()/1000000
            if memory_usage > 10:
                logging.info("Memory usage: {}, ref id = {}, len={}".format(str(memory_usage) + 'Gb', ref_group_id, len(ref_full_data)))
            ...
            if self.iteration_index == 0:
                ...
                ref_full_data[HeadersFormat.Posterior] = ref_full_data[HeadersFormat.Posterior].fillna(ref_full_data[MappingForamt.Mapp_weight])
            ...
            for base in bases:
                ...
                if prob_n_for_base[base].isna().values.any():
                    logging.warn("After computation: None in probN {} {} {}".format(base, ref_group_id, prob_n_for_base[base].values))
    ...
    def get_dfs_for_calc_probN_dask(self, bases):
        """
        get the relevant dataframe for the calculation of prob N.
        :param bases:
        :return: current_state_df, mapping_df, reads_df, posteriors_df
        ...
        if (self.iteration_index > 0):
            posteriors_df = pd.read_csv(self.paths.posteriors, index_col=False)
            posteriors_df = dd.from_pandas(posteriors_df, npartitions=8)
        ...
        return dd.from_pandas(current_state_df, npartitions=8), dd.from_pandas(mapping_df, npartitions=8), dd.from_pandas(reads_df, npartitions=8), posteriors_df


    @time_it
    def calc_prob_N_dask(self):
        """
            Pr(N=n) -> Pr(S[i] = N) for each base in each sequence, for each N {A, C, G, T}

            The result is 4 list for each sequence. ProbN_A, ProbN
_C, ProbN_G, ProbN_T
            Saved into the CurrentStateTable.
            If read or sequence is new this round (not seen at t-1), then
            there is no Pr(S|R) from previous round,
        ...
        # Get data:
        bases = [Base.A, Base.C, Base.G, Base.T]
        current_state_df, mapping_df, reads_df, posteriors_df = self.get_dfs_for_calc_probN_dask(bases)
        logging.info("length of dataframes: reads = {}, mapping = {}, current_state_df = {}"
                     .format(len(reads_df), len(mapping_df), len(current_state_df)))
        reads_data_columns, ref_data_columns, prob_success_data_columns, prob_failure_data_columns, rename_dict, prev_prob_n_columns = self.get_columns_for_calc_probN(
            bases)

        prob_n_full_dict = {Base.A: [], Base.C: [], Base.G: [], Base.T: []}

        ref_full_data = mapping_df.merge(current_state_df,
                                             on=[CurrentStateFormat.Reference_id, CurrentStateFormat.Region],
                                             how='left', suffixes=('_read', '_ref'))
        ...
        if posteriors_df is not None:
            ref_full_data = ref_full_data.merge(posteriors_df,
                                                on=[ReadsFullDataFormat.Id, CurrentStateFormat.Reference_id],
                                                how='left')
            # logging.info("len of merge of all df = {}, len(posteriors) = {}".format(len(ref_full_data), len(posteriors_df)))
        prob_n_for_base = {}
        prob_success = ref_full_data[prob_success_data_columns].copy()
        prob_success = prob_success.rename(columns=rename_dict)
        prob_fail = ref_full_data[prob_failure_data_columns].copy()
        prob_fail = prob_fail.rename(columns=rename_dict)
        mapp_weight = ref_full_data[MappingForamt.Mapp_weight]
        ...
        if self.iteration_index == 0:
            ...
            ref_full_data[HeadersFormat.Posterior] = ref_full_data[HeadersFormat.Posterior].fillna(
                ref_full_data[MappingForamt.Mapp_weight])
            posteriors = ref_full_data[HeadersFormat.Posterior]

        for base in bases:
            if self.iteration_index > 0:
                prev_prob_n = dd.read_csv(self.paths.prev_prob_n[base], blocksize=25e6)
            else:
                prob_n_cols = [HeadersFormat.Region, MappingForamt.Ref_id] + prev_prob_n_columns[base]
                prev_prob_n = pd.DataFrame(columns=prob_n_cols)
                prev_prob_n = dd.from_pandas(prev_prob_n, npartitions=8)

            ref_full_data = ref_full_data.merge(prev_prob_n,
                                                on=[CurrentStateFormat.Reference_id, CurrentStateFormat.Region],
                                                how='left')
            ref_full_data.fillna(value=1, inplace=True)
            prev_prob_n = ref_full_data[prev_prob_n_columns[base]].rename(columns=rename_dict)
        ...
        # calculate Pr(b_ik): sum_{A, C, G, T}[Pr(n_jk|b_ik)Pr(n_jk)] (for each k, i, j)
        prob_read_bases = prob_n_for_base[Base.A] + prob_n_for_base[Base.C] + prob_n_for_base[Base.G] + \
                          prob_n_for_base[Base.T]

        for base in bases:
            # for each base we calculate P(nk = base):
            if prob_n_for_base[base].isna().values.any():
                logging.warn("None in probN {} {}".format(base, prob_n_for_base[base].values))
            ...
            prob_n_for_base[base][HeadersFormat.Region] = ref_full_data[HeadersFormat.Region]
            prob_n_for_base[base][HeadersFormat.Posterior] = posteriors
            prob_n_for_base[base] = prob_n_for_base[base].groupby([CurrentStateFormat.Reference_id, CurrentStateFormat.Region]).apply(sum, meta=pd.Series())

            # Divide each sequence (reference + region) in the sum of the posteriors in the specific sequence.
            prob_n_for_base[base] = prob_n_for_base[base].divide(prob_n_for_base[base][HeadersFormat.Posterior],
                                                                 axis=0)
            ...
            if prob_n_for_base[base].isna().values.any():
                logging.warn("After computation: None in probN {} {}".format(base,
                                                                                prob_n_for_base[base].values))

            base_rename_dict = {}
            for i in range(2 * self.read_len):
                base_rename_dict[str(i)] = str(i) + base + "_prob_n"

            prob_n_for_next_itr = prob_n_for_base[base].rename(columns=base_rename_dict)
            prob_n_for_next_itr.to_csv(self.paths.prob_n[base], index=False)

            prob_n_full_dict[base] = prob_n_for_base[base].compute()
    ...
    def calc_likelihoods(self, prob_n_dict):
        ...
        logging.info("LIKELIHOOD: chunk size = {}, number of chunks = {}, mapping size = {}, reads size = {}".
    ...
    def update_references(self, probN_dict):
        ...
        for base_ix in base_ix_columns:
            ...
            try:
                prob_n_full[['2-base_' + base_ix, 'base_' + base_ix]] = arank.iloc[:, [2, 3]].applymap(lambda x: argsort_dict[x])
        ...
        new_posteriors = self._get_new_posterior(max_prob_n_full_data, full_posteriors, split_reference_suffix).drop_duplicates()
