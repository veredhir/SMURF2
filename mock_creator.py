...
class FrequencyDistribution(object):
    ...
    @staticmethod
    def is_valid(test):
        if test in [FrequencyDistribution.MUTATED, FrequencyDistribution.UNIFORM, FrequencyDistribution.RANDOM, FrequencyDistribution.POWER_LOW]:
            return True
        return False
...
def write_bacterium_to_fastq(mock_bacterium,
    ...
    with open(mock_fastq_path1, 'w') as mock_fastq1:
        with open(mock_fastq_path2, 'w') as mock_fastq2:
            read_ix = 0
            for read, pair_read in zip(reads, paired_reads):
                    for i in range(amplification_factor):
                        id1 = "@{}.{}/1\n".format(read_ix, i)
                        id2 = "@{}.{}/2\n".format(read_ix, i)
                        read_ix += 1
                        mock_fastq1.write(id1)
                        mock_fastq2.write(id2)

                        mock_fastq1.write(read + '\n')
                        mock_fastq2.write(pair_read + '\n')

                        mock_fastq1.write('+\n')
                        mock_fastq2.write('+\n')

                        # length = 126, '~' represent the smallest allowed error prob
                        mock_fastq1.write(''.join(126*['J']) + '\n')
                        mock_fastq2.write(''.join(126*['J']) + '\n')
...
def get_frequency_list(amount_of_refs=15, max_percent=20, min_frequency=0.5, isSorted=False, distribution=FrequencyDistribution.UNIFORM,
                       keep_original=False, bacterium_to_change=0):
    frequencies = []
    ...
    elif distribution == FrequencyDistribution.MUTATED and keep_original==True:
        frequencies = [100 / (amount_of_refs+bacterium_to_change)] * amount_of_refs
        for i in range(bacterium_to_change):
            frequencies[i*2] = 2*frequencies[i*2]
        frequencies = frequencies[:amount_of_refs]
...
def create_mock_reads(fasta_directory,
    ...
    mock_fastq1 = os.path.join(base_fastq + "_R1.fq")
    mock_fastq2 = os.path.join(base_fastq + "_R2.fq")
    ...
    write_bacterium_to_fastq(mock_bacterium, mock_fastq1, mock_fastq2)
...
def create_mock_reads_fa(fasta_directory,
    ...
    base_read_file_name = 'mockReads_R'
    mock_fa1 = os.path.join(mock_reads_dir, base_read_file_name + "1.fa")
    mock_fq1 = os.path.join(mock_reads_dir, base_read_file_name + "1.fq")
    mock_fq2 = os.path.join(mock_reads_dir, base_read_file_name + "2.fq")
    expected_data_path = os.path.join(mock_reads_dir, "expected_res.csv")
    ...
    amplification_factor = (total_amout_of_reads/total_num_of_regions)/total_number_of_bacterias

    write_bacterium_to_fasta(mock_bacterium, mock_fa1, min_frequency)
    write_bacterium_to_fastq(mock_bacterium, mock_fq1, mock_fq2, amplification_factor=amplification_factor)
    write_mock_bacterium_to_csv(mock_bacterium, expected_data_path)
...
def get_command_line_arguments_parser():
    ...
    group_reads.add_option("--fq", dest="create_fastq_size",
                           type="int", default=0,
                           help="The number of repeats of each read in the fastq files [default: %default]")
